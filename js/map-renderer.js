...
var getCustomer = function (customerId, file) {
  ...
  for (idx in file) {
    ...
    if (
      ...
      !(cust["BRANCH_ID"] in branches_visited)
    ) {
...
var visCustomer = function (custInfo, map, markerList) {
  ...
  for (idx in custInfo["visitedBranch"]) {
    ...
    var marker = new L.marker([info["lat"], info["lon"]], {
        icon: branchIcon
      })
      .bindPopup(idx)
      .addTo(map);
  ...
  var marker = L.marker(
      [custInfo["homeLoc"]["lat"], custInfo["homeLoc"]["lon"]], {
        icon: homeIcon
      }
    )
    .bindPopup("Home")
  ...
  var marker = L.marker(
      [custInfo["workLoc"]["lat"], custInfo["workLoc"]["lon"]], {
        icon: workIcon
      }
    )
    .bindPopup("Work")
...
var getBranch = function (branchId, file) {
  ...
  for (idx in file) {
    cust = file[idx];
    if (
      cust["BRANCH_ID"] == branchId &&
      !(cust["CUSTOMER_ID"] in visitedCustomers)
    ) {
...
var visBranch = function (branchInfo, map, markerList) {
  ...
  for (idx in branchInfo["visitedCustomers"]) {
    ...
    var marker = new L.marker(homePoint, {
        icon: homeIcon
      })
      .bindPopup(idx)
      .addTo(map);
    ...
    var marker = new L.marker(workPoint, {
        icon: workIcon
      })
      .bindPopup(idx)
      .addTo(map);
  ...
  var marker = L.marker([branchInfo["loc"]["lat"], branchInfo["loc"]["lon"]], {
      icon: branchIcon
    })
...
var customerHsitory = function (customerId, file) {
  var visitHistory = new Array(168 + 1)
    .join("0")
    .split("")
    .map(parseFloat);
  ...
  for (idx in file) {
    ...
    if (cust["CUSTOMER_ID"] == customerId) {
      ...
      var dateVisit = new Date(
        dateString.slice(2, 5) +
        dateString.slice(0, 2) +
        "," +
        dateString.slice(5, 9) +
        " " +
        dateString.slice(10)
      );
